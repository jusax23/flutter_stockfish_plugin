   ...
   public:
    static void start(const std::string& fname) {
        ...
        if (!fname.empty())
        {
            ...
            if (!l.file.is_open())
            {
                std::cerr << "Unable to open debug log file " << fname << fakeendl;
                exit(EXIT_FAILURE);
            }
...
void dbg_print() {
    ...
    for (int i = 0; i < MaxDebugSlots; ++i)
        if ((n = mean[i][0]))
        {
            std::cerr << "Mean #" << i << ": Total " << n << " Mean " << E(mean[i][1]) << fakeendl;
        }
    ...
    for (int i = 0; i < MaxDebugSlots; ++i)
        if ((n = stdev[i][0]))
        {
            ...
            std::cerr << "Stdev #" << i << ": Total " << n << " Stdev " << r << fakeendl;
        }
    ...
    for (int i = 0; i < MaxDebugSlots; ++i)
        if ((n = correl[i][0]))
        {
            ...
            std::cerr << "Correl. #" << i << ": Total " << n << " Coefficient " << r << fakeendl;
        }
}
...
// Used to serialize access to fakeout
